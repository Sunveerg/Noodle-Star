...
const ReviewList: React.FC = (): JSX.Element => {
    ...
    const generateStars = (rating: number) => {
        ...
        for (let i = 0; i < 5; i++) {
            if (i < rating) {
                stars.push(<span key={i} className="star filled">★</span>);  // Filled star (gold)
            } else {
                stars.push(<span key={i} className="star empty">★</span>);   // Empty star (gray)
            }
        }
        return stars;
    };
    ...
    return (
        <div className="titleSection">
            <h2 className="pageTitle">Customer Reviews
            <img
                src={noodleImg}
                alt="Noodle"
